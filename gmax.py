        ...
        for tile in ai_tiles:
            for direction in ["left", "right"]:
                if evaluate_move(board, tile, direction):
                    ...
                    eval = minimax(
                        new_board, new_ai_tiles, player_tiles, stock, depth - 1, False
                    )
        ...
        for tile in player_tiles:
            for direction in ["left", "right"]:
                if evaluate_move(board, tile, direction):
                    ...
                    eval = minimax(
                        new_board, ai_tiles, new_player_tiles, stock, depth - 1, True
                    )
...
class DominoGame:
    ...
    def setup_ui(self):
        self.opponent_frame = tk.Frame(
            self.root, bg="#ffcccb", height=100, relief=tk.SUNKEN, bd=2
        )
        ...
        self.stock_frame = tk.Frame(
            self.main_frame, bg="#e0e0e0", width=300, relief=tk.RAISED, bd=2
        )
        self.stock_frame.pack(side=tk.RIGHT, fill=tk.Y, padx=10, pady=10)

        self.player_frame = tk.Frame(
            self.root, bg="#fdfd96", height=150, relief=tk.SUNKEN, bd=2
        )
    ...
    def update_ui(self):
        for widget in self.board_frame.winfo_children():
            widget.destroy()
        tk.Label(
            self.board_frame, text="Board:", font=("Arial", 16, "bold"), bg="#d0e6f5"
        ...
        if self.board:
            ...
            tk.Label(
                self.board_frame, text="Empty", font=("Arial", 16), bg="#d0e6f5"
            ).pack(anchor="nw", padx=10)
        ...
        for idx, tile in enumerate(self.player_tiles):
            btn = tk.Button(
                ...
                text=f"[{tile[0]}|{tile[1]}]",
                font=("Arial", 14),
                command=lambda i=idx: self.play_tile(i),
                bg="#d1e7dd",
                relief=tk.RAISED,
            )
            btn.pack(side=tk.LEFT, padx=5, pady=5)
        ...
        for widget in self.stock_frame.winfo_children():
            widget.destroy()
        tk.Label(
            self.stock_frame, text="Stock:", font=("Arial", 16, "bold"), bg="#e0e0e0"
        ).pack()
        for _ in self.stock:
            btn = tk.Button(
                self.stock_frame,
        ...
        self.draw_button["state"] = (
            tk.NORMAL
            if not self.has_valid_move(self.player_tiles) and self.stock
            else tk.DISABLED
        )
        self.pass_button["state"] = (
            tk.NORMAL
    ...
    def play_tile(self, idx):
        ...
        if self.board:
            if evaluate_move(self.board, tile, "left") and evaluate_move(
                self.board, tile, "right"
            ):
            ...
            if direction == "left":
                self.board.insert(
                    0, tile if tile[1] == self.board[0][0] else tile[::-1]
                )
            else:
    ...
    def ask_direction(self, tile):
        ...
        tk.Button(
            popup, text="Left", command=lambda: [direction.set("left"), popup.destroy()]
        ).pack(side=tk.LEFT, padx=20)
        tk.Button(
            popup,
            text="Right",
            command=lambda: [direction.set("right"), popup.destroy()],
    ...
    def has_valid_move(self, tiles):
        for tile in tiles:
            if evaluate_move(self.board, tile, "left") or evaluate_move(
                self.board, tile, "right"
            ):
                return True
        return False
